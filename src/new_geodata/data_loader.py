'''
https://github.com/THUDM/HGB/blob/master/NC/benchmark/scripts/data_loader.py
https://github.com/THUDM/HGB/blob/master/NC/benchmark/methods/baseline/utils/data.py
'''
...
class data_loader:
    ...
    def evaluate(self, pred, *args):
        ...
        if len(args) == 2:
            id_bucket, n_buckets = args
            var = np.var([f1_score(t, p) for t, p in zip(y_true, pred.numpy())])
            pos = np.zeros(n_buckets)
            tot = np.zeros(n_buckets)
            for i, (t, p) in enumerate(zip(y_true, pred)):
                pos[id_bucket[i]] += f1_score(t, p)
                tot[id_bucket[i]] += 1
            tot[tot == 0] = 1
            result = {"micro-f1": micro, "macro-f1": macro, 'var': var, 'var_b': np.var(pos / tot)}
        elif len(args) == 0:
    ...
    def load_links(self):
        ...
        with open(os.path.join(self.path, "link.dat"), "r", encoding="utf-8") as f:
            for line in f:
                ...
                if r_id not in edge_index_dict:
                    edge_index_dict[r_id] = [[h_id], [t_id]]
                else:
                    edge_index_dict[r_id][0].append(h_id)
                    edge_index_dict[r_id][1].append(t_id)
        ...
        self.edge_index_dict = {k: torch.tensor(v) for k, v in edge_index_dict.items()}
        new_data = {}
...
def load_data(root, name_, val_ratio=0.2):
    dl = data_loader(root)
    features = []
    for i in range(len(dl.nodes["count"])):
        ...
        if th is None:
            ...
            features.append(torch.eye(dl.nodes["count"][i]).float().to_sparse())
        else:
            features.append(torch.tensor(th).float())
    adjM = sum(dl.links["data"].values())
    labels = np.zeros((dl.nodes["count"][0], dl.labels_train["num_classes"]), dtype=int)  # assume class 0 is target
